                                  ...
                                  DirectionalLightData lightData)
{
	...
	return ComputePhongLighting(albedo, normalW, toEye, lightVector, lightData.Color.rgb, roughness, metallic);
}
                            ...
                            PointLightData lightData)
{
	...
	return ComputePhongLighting(albedo, normalW, toEye, lightVector, attenuation * lightData.Color.rgb, roughness, metallic);
}
                           ...
                           SpotLightData lightData)
{
	...
	return ComputePhongLighting(albedo, normalW, toEye, lightVector, attenuation * spotFactor * lightData.Color.rgb, roughness,
	                            metallic);
}

float4 PhongLightingPS(FullscreenVertexOut pin) : SV_Target
{
	const float3 backgroundColor = gBackgroundColor.rgb;
	const int2 pixelCoord = int2(pin.PosH.xy);
	const float4 albedoSample = gTexture0.Load(int3(pixelCoord, 0));
	if (albedoSample.a < 0.001f)
	{
		return float4(ApplyOutputTransform(SampleSkybox(pin.TexC)), 1.0f);
	}

	const float4 normalSample = gTexture1.Load(int3(pixelCoord, 0));
	const float depthSample = gDepthBuffer.Load(int3(pixelCoord, 0)).r;
	const float3 normalW = normalize(normalSample.xyz * 2.0f - 1.0f);
	const float3 posW = ReconstructWorldPosition(pixelCoord, depthSample);
	const float4 pbrParams = gTexture2.Load(int3(pixelCoord, 0));
	const int debugViewMode = (int)round(gAuxiliarySettings.x);
	const float opacity = saturate(albedoSample.a);
	...
	if (debugViewMode == 2)
	{
		return float4(lerp(backgroundColor, normalW * 0.5f + 0.5f, opacity), 1.0f);
	}
	...
	if (debugViewMode == 4)
	{
		return float4(lerp(backgroundColor, GetCascadeDebugColor(shadowCascadeIndex), opacity), 1.0f);
	}
	if (debugViewMode == 5)
	{
		return float4(lerp(backgroundColor, directionalShadowFactor.xxx, opacity), 1.0f);
	}
	if (debugViewMode == 8)
	{
		return float4(lerp(backgroundColor, pbrParams.x.xxx, opacity), 1.0f);
	...
	const float3 ambient =
	    (gAmbientLight.rgb * gAmbientLight.w * albedoSample.rgb + gAmbientFloor.rgb * gAmbientFloor.w * albedoSample.rgb) *
	    ambientOcclusion;
	float3 directionalLighting = 0.0f;
	float3 pointLighting = 0.0f;
	float3 spotLighting = 0.0f;
	...
	[unroll] for (uint lightIndex = 0; lightIndex < DIRECTIONAL_LIGHT_COUNT; ++lightIndex)
	{
		directionalLighting += directionalShadowFactor *
		                       ApplyPhongDirectionalLight(albedoSample.rgb, normalW, toEye, roughness, metallic,
		                                                  gDirectionalLights[lightIndex]);
	}
	[unroll] for (uint lightIndex = 0; lightIndex < POINT_LIGHT_COUNT; ++lightIndex)
	{
		pointLighting += ApplyPhongPointLight(albedoSample.rgb, normalW, toEye, posW, roughness, metallic, gPointLights[lightIndex]);
	}
	[unroll] for (uint lightIndex = 0; lightIndex < SPOT_LIGHT_COUNT; ++lightIndex)
	{
		spotLighting += ApplyPhongSpotLight(albedoSample.rgb, normalW, toEye, posW, roughness, metallic, gSpotLights[lightIndex]);
	}
	...
	const float3 finalColor = lerp(backgroundColor, ambient + directLighting, opacity);
	return float4(ApplyOutputTransform(finalColor), 1.0f);
}
