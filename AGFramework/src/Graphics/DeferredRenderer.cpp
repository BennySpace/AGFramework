...
DeferredRenderer::ObjectConstants DeferredRenderer::BuildObjectConstants(const FrameData &frameData) const
{
	...
	const XMMATRIX texTransform = XMLoadFloat4x4(&frameData.TexTransform);
	...
	objectConstants.ShadowCascadeSplits =
	    XMFLOAT4(frameData.CascadedShadowData.SplitDistances[0], frameData.CascadedShadowData.SplitDistances[1],
	             frameData.CascadedShadowData.SplitDistances[2], frameData.CascadedShadowData.SplitDistances[3]);
	...
	objectConstants.ShadowSettings0 =
	    XMFLOAT4(static_cast<float>(cascadeCount), frameData.ShadowSettings.PcfRadius, frameData.ShadowSettings.ShadowStrength,
...
void DeferredRenderer::BuildCascadedShadowMap(DirectX12Context &context)
{
	...
	if (m_cascadedShadowMap != nullptr)
	{
		...
		if (currentDesc.Width == shadowMapDesc.Width && currentDesc.Height == shadowMapDesc.Height &&
		    currentDesc.CascadeCount == shadowMapDesc.CascadeCount && currentDesc.ResourceFormat == shadowMapDesc.ResourceFormat &&
		    currentDesc.DsvFormat == shadowMapDesc.DsvFormat && currentDesc.SrvFormat == shadowMapDesc.SrvFormat)
		{
			return;
		}
	}
                                                   ...
                                                   D3D12_RESOURCE_STATES afterState)
{
	...
	D3D12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(m_cascadedShadowMap->GetResource(), beforeState, afterState);
...
{
	CD3DX12_STATIC_SAMPLER_DESC linearWrapSampler(0, D3D12_FILTER_MIN_MAG_MIP_LINEAR, D3D12_TEXTURE_ADDRESS_MODE_WRAP,
	                                              D3D12_TEXTURE_ADDRESS_MODE_WRAP, D3D12_TEXTURE_ADDRESS_MODE_WRAP);
	CD3DX12_STATIC_SAMPLER_DESC shadowComparisonSampler(
	    1, D3D12_FILTER_COMPARISON_MIN_MAG_LINEAR_MIP_POINT, D3D12_TEXTURE_ADDRESS_MODE_BORDER, D3D12_TEXTURE_ADDRESS_MODE_BORDER,
	    D3D12_TEXTURE_ADDRESS_MODE_BORDER, 0.0f, 16, D3D12_COMPARISON_FUNC_LESS_EQUAL, D3D12_STATIC_BORDER_COLOR_OPAQUE_WHITE);
	CD3DX12_STATIC_SAMPLER_DESC linearClampSampler(2, D3D12_FILTER_MIN_MAG_MIP_LINEAR, D3D12_TEXTURE_ADDRESS_MODE_CLAMP,
	...
	ThrowIfFailed(D3D12SerializeRootSignature(&geometryRootSigDesc, D3D_ROOT_SIGNATURE_VERSION_1, serializedRootSig.GetAddressOf(),
	...
	CD3DX12_ROOT_SIGNATURE_DESC lightingRootSigDesc(3, lightingRootParameters, _countof(lightingStaticSamplers), lightingStaticSamplers,
	                                                D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);

	serializedRootSig.Reset();
	errorBlob.Reset();
	ThrowIfFailed(D3D12SerializeRootSignature(&lightingRootSigDesc, D3D_ROOT_SIGNATURE_VERSION_1, serializedRootSig.GetAddressOf(),
	                                          errorBlob.GetAddressOf()));
	...
	serializedRootSig.Reset();
	errorBlob.Reset();
	ThrowIfFailed(D3D12SerializeRootSignature(&shadowRootSigDesc, D3D_ROOT_SIGNATURE_VERSION_1, serializedRootSig.GetAddressOf(),
	                                          errorBlob.GetAddressOf()));
...
void DeferredRenderer::BuildPSO(DirectX12Context &context)
{
	...
	geometryPsoDesc.VS = {reinterpret_cast<BYTE *>(m_shaders["standardVS"]->GetBufferPointer()), m_shaders["standardVS"]->GetBufferSize()};
	geometryPsoDesc.PS = {reinterpret_cast<BYTE *>(m_shaders["gbufferPS"]->GetBufferPointer()), m_shaders["gbufferPS"]->GetBufferSize()};
	...
	lightingPsoDesc.VS = {reinterpret_cast<BYTE *>(m_shaders["fullscreenVS"]->GetBufferPointer()),
	                      m_shaders["fullscreenVS"]->GetBufferSize()};
	lightingPsoDesc.PS = {reinterpret_cast<BYTE *>(m_shaders["deferredLightingPS"]->GetBufferPointer()),
...
{
	if (m_directionalShadowOpaquePSO != nullptr && m_directionalShadowAlphaCutoutPSO != nullptr &&
	...
	shadowPsoDesc.VS = {reinterpret_cast<BYTE *>(m_shaders["shadowVS"]->GetBufferPointer()), m_shaders["shadowVS"]->GetBufferSize()};
	...
	shadowPsoDesc.PS = {reinterpret_cast<BYTE *>(m_shaders["shadowAlphaCutoutPS"]->GetBufferPointer()),
